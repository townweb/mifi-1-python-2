...
from primitive_db.storage import (
    delete_table_file,
    load_metadata,
    load_table_data,
    save_metadata,
...
def update_rows(table_name, set_clause, where_clause, cacher):
    ...
    count = 0
    for r in rows:
        if _row_matches(r, typed_where):
            for k, v in typed_set.items():
                r[k] = v
            count += 1
...
def _ensure_columns_exist(columns, clause):
    for k in clause.keys():
        _type_of_column(columns, k)


def _coerce_clause(columns, clause):
    result = {}
    for k, v in clause.items():
        col_type = _type_of_column(columns, k)
        result[k] = _coerce_value(v, col_type)
    return result
    ...
    if expected_type == "int":
        ...
        if isinstance(value, str):
            try:
                ...
                raise ValidationError(f"Ожидался int, получено: {value}") from e
...
def _select_impl(table_name, where):
    ...
    if not where:
        return rows
    return [r for r in rows if _row_matches(r, where)]
...
def _print_rows(columns, rows):
    ...
    for r in rows:
        t.add_row([_to_display(r.get(name)) for name in field_names])
